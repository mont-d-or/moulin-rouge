...
const MonthSvg = ({daysInMonth, daysWithPeriod}: MonthSvgProps) => {
  ...
  const spacing = 2
  return (
  <svg width="500" height="20" viewBox="0 0 300 15" xmlns="http://www.w3.org/2000/svg">
    {[...Array(daysInMonth).keys()].map((dayInMonth) => (<rect rx="2" key={dayInMonth} className={daysWithPeriod.includes(dayInMonth) ? 'red': 'green'} x={dayInMonth*(dayWidth + spacing)} width={dayWidth} height={dayHeight} shape-rendering="geometricPrecision"/>))}
  </svg>)
}
...
const PeriodCalendar = ({ history }: Props) => {
  ...
  history.forEach(element => {
    ...
    if (element.endDate)
      ...
      lastEndDate = maxDate(moment.utc(element.startDate), lastEndDate)
  })
  ...
  while (currentMonth < endOfPeriod) {
    months.push(
      <div key={JSON.stringify(currentMonth)}>
        <MonthSvg daysInMonth={currentMonth.daysInMonth()} daysWithPeriod={[6,7,8,9,10]}></MonthSvg>
  ...
  return (
    <div>
      <div className="history-item shared-flex-row">
        There are {history?.length} items.<br/>
      </div>
      <div>
      { months }
      </div>
